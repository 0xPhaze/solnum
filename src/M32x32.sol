...
using M32x32Lib for M32x32 global;
...
library M32x32Lib {
    ...
    function M32x32Header(uint256 ptr, uint256 n, uint256 m) internal pure returns (M32x32 A) {
    ...
    function header(M32x32 A) internal pure returns (uint256 n, uint256 m, uint256 ptr) {
        n = (M32x32.unwrap(A)) & __UM256.MAX_24_BITS;
        m = (M32x32.unwrap(A) >> 24) & __UM256.MAX_24_BITS;
        ptr = (M32x32.unwrap(A) >> 48) & N32x32Lib.UINT64_MAX;
    }

    function shape(M32x32 A) internal pure returns (uint256 n, uint256 m) {
        n = (M32x32.unwrap(A)) & __UM256.MAX_24_BITS;
        m = (M32x32.unwrap(A) >> 24) & __UM256.MAX_24_BITS;
    }

    function dim0(M32x32 A) internal pure returns (uint256 n) {
        n = (M32x32.unwrap(A)) & __UM256.MAX_24_BITS;
    }

    function dim1(M32x32 A) internal pure returns (uint256 m) {
        m = (M32x32.unwrap(A) >> 24) & __UM256.MAX_24_BITS;
    }

    function length(M32x32 A) internal pure returns (uint256 len) {
        unchecked {
    ...
    function sizeBytes(M32x32 A) internal pure returns (uint256 size) {
        unchecked {
            uint256 n = (M32x32.unwrap(A)) & __UM256.MAX_24_BITS;
            uint256 m = (M32x32.unwrap(A) >> 24) & __UM256.MAX_24_BITS;
    ...
    function ref(M32x32 A) internal pure returns (uint256 ptr) {
        ptr = (M32x32.unwrap(A) >> 48) & UINT64_MAX;
    }

    /* ------------- constructors ------------- */

    function mallocLike(M32x32 A) internal pure returns (M32x32 C) {
        (uint256 n, uint256 m) = shape(A);

        C = mallocM32x32(n, m);
    }

    function mallocM32x32(uint256 n, uint256 m) internal pure returns (M32x32 A) {
        unchecked {
    ...
    function zeros(uint256 n, uint256 m) internal pure returns (M32x32 C) {
        C = mallocM32x32(n, m); // Allocate memory for matrix.

        // Fill matrix with `0`s.
        fill_(C, N32x32Lib.ZERO);
    }

    function ones(uint256 n, uint256 m) internal pure returns (M32x32 C) {
        // Allocate memory for matrix.
        C = mallocM32x32(n, m);

        // Fill matrix with `1`s.
        fill_(C, N32x32Lib.ONE);
    }

    function eye(uint256 n, uint256 m) internal pure returns (M32x32 C) {
        unchecked {
            ...
            N32x32 one = N32x32Lib.ONE;

            if (ptr != end) {
                assembly {
                    let preserveWord := and(mload(ptr), not(UINT64_MAX))

                    mstore(ptr, or(one, preserveWord)) // Store `1` at `ptr`, preserve existing word.
                }
    ...
    function range(uint256 end) internal pure returns (M32x32 C) {
        return range(0, end);
    }
    ...
    function range(uint256 start, uint256 end) internal pure returns (M32x32 C) {
        unchecked {
            // `start <= end` must hold.
    ...
    function reshape(M32x32 A, uint256 nNew, uint256 mNew) internal pure returns (M32x32 out) {
        unchecked {
    ...
    function atIndex(M32x32 A, uint256 index) internal pure returns (N32x32 a) {
        unchecked {
    ...
    function setIndex(M32x32 A, uint256 index, N32x32 value) internal pure {
        unchecked {
            (uint256 n, uint256 m, uint256 ptr) = header(A);
    ...
    function at(M32x32 A, uint256 i, uint256 j) internal pure returns (N32x32 a) {
        unchecked {
            (uint256 n, uint256 m, uint256 ptr) = header(A);
    ...
    function set(M32x32 A, uint256 i, uint256 j, N32x32 value) internal pure {
        unchecked {
            (uint256 n, uint256 m, uint256 ptr) = header(A);
    ...
    function setUnsafe(M32x32 A, uint256 i, uint256 j, N32x32 value) internal pure {
        unchecked {
    ...
    function addUnchecked(M32x32 A, M32x32 B) internal pure returns (M32x32 C) {
    ...
    function addUncheckedTo_(M32x32 A, M32x32 B, M32x32 C) internal pure {
        unchecked {
    ...
    function add(M32x32 A, M32x32 B) internal pure returns (M32x32 C) {
        (uint256 n, uint256 m) = A.shape();
    ...
    function addTo_(M32x32 A, M32x32 B, M32x32 C) internal pure {
        unchecked {
            (uint256 nA, uint256 mA, uint256 ptrA) = header(A);
            (uint256 nB, uint256 mB, uint256 ptrB) = header(B);

            // todo: check C's dimensions
            if (nA != nB || mA != mB) revert M32x32_IncompatibleDimensions();
            ...
            // if (overflow > UINT64_MAX) revert N32x32Lib.Overflow(); // TODO: inspect why this also works?
        }
    }

    function eq(M32x32 A, M32x32 B) internal pure returns (M32x32 C) {
        C = mallocLike(A);

        eqTo_(A, B, C);
    }

    function eqTo_(M32x32 A, M32x32 B, M32x32 C) internal pure {
        unchecked {
            (uint256 nA, uint256 mA, uint256 ptrA) = header(A);
            (uint256 nB, uint256 mB, uint256 ptrB) = header(B);
            (uint256 nC, uint256 mC, uint256 ptrC) = header(C);

            if (nA != nB || mA != mB || nA != nC || mA != mC) revert M32x32_IncompatibleDimensions();

            // Loop over all `n * m` elements of 8 bytes.
            uint256 size = nA * mA * 8;
            // Round up to parse full words (4 elements).
            uint256 endA = ptrA + ((size + 31) & ~uint256(31));
            ...
            while (ptrA != endA) {
                assembly {
                    ...
                    cX4 := eq(and(a, UINT64_MAX), and(b, UINT64_MAX))
                    cX4 := or(cX4, shl(64, eq(and(a, shl(64, UINT64_MAX)), and(b, shl(64, UINT64_MAX)))))
                    cX4 := or(cX4, shl(128, eq(and(a, shl(128, UINT64_MAX)), and(b, shl(128, UINT64_MAX)))))
                    cX4 := or(cX4, shl(192, eq(shr(192, a), shr(192, b))))
    ...
    function addBroadcast(M32x32 A, M32x32 B) internal pure returns (M32x32 C) {
        (uint256 nA, uint256 mA) = shape(A);
        (uint256 nB, uint256 mB) = shape(B);

        uint256 n = nA > nB ? nA : nB;
        uint256 m = mA > mB ? mA : mB;

        // Allocate new matrix of the same dimensions.
        C = mallocM32x32(n, m);

        // Add scalar `A` and `B`, store result in `C`.
        addBroadcastTo_(A, B, C);
    }

    /// @dev Only broadcasts `B`.
    function addBroadcastTo_(M32x32 A, M32x32 B, M32x32 C) internal pure {
        unchecked {
            (uint256 nA, uint256 mA, uint256 ptrA) = header(A);
            (uint256 nB, uint256 mB, uint256 refB) = header(B);

            if (mA & 0x3 != 0 || nB != 1) {
                revert M32x32_NotImplemented();
            }

            if (nB != 1 || mA != mB || mA & 0x3 != 0) {
                revert M32x32_IncompatibleDimensions();
            }
            ...
            uint256 size = nA * mA * 8;
            // Up until here we can load & parse full words (4 elements).
            uint256 endC = ptrC + (size & ~uint256(31));
            // The rest needs to be parsed individually.
            // uint256 rest = size & 31;
            ...
            while (ptrC != endC) {
                ptrB = refB;
                uint256 endB = ptrB + (mB * 8 & ~uint256(31));
                ...
                while (ptrB != endB) {
                    assembly {
                        ...
                        // Overflow when signs of `a` and `b` are equal, but differ from `c`.
                        carry := or(carry, and(and(not(xor(a, b)), xor(a, c)), INT64_SIGN_X4))
                    }
                    // Advance pointers to the next slot.
                    ptrA = ptrA + 32;
                    ptrB = ptrB + 32;
            ...
            if (carry != 0) revert N32x32Lib.Overflow();
        }
    }
    ...
    /// `ptrA = 8 * (k + i * mA)`
    /// `ptrB = 8 * (j + k * mb)`
    function dot(M32x32 A, M32x32 B) internal pure returns (M32x32 C) {
        unchecked {
            (uint256 nA, uint256 mA, uint256 ptrA) = header(A);
            (uint256 nB, uint256 mB, uint256 ptrB) = header(B);
    ...
    function dotTransposedX4(M32x32 A, M32x32 B) internal pure returns (M32x32 C) {
        unchecked {
            (uint256 nA, uint256 mA, uint256 ptrA) = header(A);
            (uint256 nB, uint256 mB, uint256 ptrB) = header(B);
    ...
    function dotX4(M32x32 A, M32x32 B) internal pure returns (M32x32 C) {
        unchecked {
            (uint256 nA, uint256 mA, uint256 ptrA) = header(A);
            (uint256 nB, uint256 mB, uint256 ptrB) = header(B);
    ...
    /// @dev Computes `C_ij = A_ik B_jk`
    /// Implicitly transposes B by calculating A @ B^T
    function dotTransposed(M32x32 A, M32x32 B) internal pure returns (M32x32 C) {
        unchecked {
            ...
            if (overflow > UINT64_MAX) revert N32x32Lib.Overflow();
        }
    }

    /// @dev Computes `C_ij = max(A_ik B_jk + b_1j, 0)`
    function linearReluTransposed(M32x32 A, M32x32 B, M32x32 bias) internal pure returns (M32x32 C) {
        unchecked {
            (uint256 nA, uint256 mA, uint256 refA) = header(A);
            (uint256 nB, uint256 mB, uint256 refB) = header(B);
            ...
            if (overflow > uint256(INT64_MAX)) revert N32x32Lib.Overflow();
        }
    }

    /// @dev Computes `C_ij = A_ik B_jk + b_1j`
    function linearTransposed(M32x32 A, M32x32 B, M32x32 bias) internal pure returns (M32x32 C) {
        unchecked {
            (uint256 nA, uint256 mA, uint256 refA) = header(A);
            (uint256 nB, uint256 mB, uint256 refB) = header(B);
            (uint256 nb, uint256 mb, uint256 refb) = header(bias);
    ...
    function eqAll(M32x32 A, M32x32 B) internal pure returns (bool equals) {
        unchecked {
    ...
    function addScalarUnchecked(M32x32 A, N32x32 s) internal pure returns (M32x32 C) {
        (uint256 n, uint256 m) = A.shape();
    ...
    function addScalar(M32x32 A, N32x32 s) internal pure returns (M32x32 C) {
        (uint256 n, uint256 m) = A.shape();
    ...
    function addScalarUncheckedTo_(M32x32 A, N32x32 s, M32x32 C) internal pure {
        unchecked {
    ...
    function addScalarTo_(M32x32 A, N32x32 s, M32x32 C) internal pure {
        unchecked {
            (uint256 n, uint256 m, uint256 ptrA) = header(A);
    ...
    function mulIntUnchecked(M32x32 A, int256 s) internal pure returns (M32x32 C) {
        (uint256 n, uint256 m) = A.shape();
    ...
    function mulInt(M32x32 A, int256 s) internal pure returns (M32x32 C) {
        (uint256 n, uint256 m) = A.shape();
        ...
        // Multiply `A` with scalar `s`, store result in `C`.
        mulScalarTo_(A, N32x32Lib.fromInt(s), C);
    }

    function mulScalarUnchecked(M32x32 A, N32x32 s) internal pure returns (M32x32 C) {
        (uint256 n, uint256 m) = A.shape();
    ...
    function mulScalar(M32x32 A, N32x32 s) internal pure returns (M32x32 C) {
        (uint256 n, uint256 m) = A.shape();
    ...
    function mulScalarUncheckedTo_(M32x32 A, N32x32 s, M32x32 C) internal pure {
        unchecked {
            (uint256 n, uint256 m, uint256 ptrA) = header(A);
    ...
    function mulScalarTo_(M32x32 A, N32x32 s, M32x32 C) internal pure {
        unchecked {
            (uint256 n, uint256 m, uint256 ptrA) = header(A);
            ...
            if (carry > UINT64_MAX) revert N32x32Lib.Overflow();
        }
    }

    /* ------------- Mat operators ------------- */

    function min(M32x32 A) internal pure returns (N32x32 minValue) {
        unchecked {
            (uint256 n, uint256 m, uint256 ptr) = header(A);

            minValue = N32x32Lib.MAX; // Set current min to `MAX`.
    ...
    function max(M32x32 A) internal pure returns (N32x32 maxValue) {
        unchecked {
            (uint256 n, uint256 m, uint256 ptr) = header(A);

            maxValue = N32x32Lib.MIN; // Set current max to `MIN`.
    ...
    function minMax(M32x32 A) internal pure returns (N32x32 minValue, N32x32 maxValue) {
        unchecked {
            (uint256 n, uint256 m, uint256 ptr) = header(A);
    ...
    /// @dev Unchecked
    function sum(M32x32 A) internal pure returns (N32x32 s) {
        unchecked {
            (uint256 n, uint256 m, uint256 ptr) = header(A);
    ...
    /// @dev Unchecked
    function sumEq(M32x32 A, M32x32 B) internal pure returns (N32x32 s) {
        unchecked {
            (uint256 nA, uint256 mA, uint256 ptrA) = header(A);
            (uint256 nB, uint256 mB, uint256 ptrB) = header(B);
            ...
            while (ptrA != endA) {
                assembly {
                    ...
                    s := add(s, eq(and(a, UINT64_MAX), and(b, UINT64_MAX)))
                    s := add(s, eq(and(a, shl(64, UINT64_MAX)), and(b, shl(64, UINT64_MAX))))
                    s := add(s, eq(and(a, shl(128, UINT64_MAX)), and(b, shl(128, UINT64_MAX))))
                    s := add(s, eq(shr(192, a), shr(192, b)))
                }

                ptrA = ptrA + 32; // Advance pointer to the next slot.
                ptrB = ptrB + 32; // Advance pointer to the next slot.
            }
            ...
            if (rest != 0) {
                ...
                assembly {
                    ...
                    bX4 := mload(ptrB) // Load packed elements at `ptrB`.
                }

                // Parse the last remaining word in chunks of 8 bytes.
                while (true) {
                    assembly {
                        let a := signextend(7, shr(mul(sub(32, rest), 8), aX4))
                        let b := signextend(7, shr(mul(sub(32, rest), 8), bX4))
    ...
    /// @dev Unchecked
    function mean(M32x32 A) internal pure returns (N32x32 s) {
        unchecked {
            (uint256 n, uint256 m) = shape(A);

            s = A.sum().div(N32x32Lib.fromUint(n * m));
        }
    }
    ...
    /// @dev Unchecked
    function vari(M32x32 A) internal pure returns (N32x32 variance) {
        unchecked {
            (uint256 n, uint256 m, uint256 ptr) = header(A);

            uint256 len = n * m;

            if (len < 2) return N32x32Lib.ZERO;
    ...
    function eqAllScalar(M32x32 A, N32x32 s) internal pure returns (bool equals) {
        unchecked {
            (uint256 n, uint256 m, uint256 ptr) = header(A);
    ...
    function gtAllScalar(M32x32 A, N32x32 s) internal pure returns (bool gtResult) {
        unchecked {
            if (s.eq(N32x32Lib.MAX)) return gtResult = false; // Exit early.
    ...
    function gteAllScalar(M32x32 A, N32x32 s) internal pure returns (bool gteResult) {
        unchecked {
            if (s.eq(N32x32Lib.MIN)) return true; // Exit early.

            gteResult = gtAllScalar(A, s.sub(N32x32.wrap(1))); // Reduce `s` so we can use `gt`.
        }
    }

    function ltAllScalar(M32x32 A, N32x32 s) internal pure returns (bool ltResult) {
        unchecked {
            if (s.eq(N32x32Lib.MIN)) return ltResult = false; // Exit early.
    ...
    function lteAllScalar(M32x32 A, N32x32 s) internal pure returns (bool lteResult) {
        unchecked {
            if (s.eq(N32x32Lib.MAX)) return lteResult = true; // Exit early.

            lteResult = ltAllScalar(A, s.add(N32x32.wrap(1))); // Increase by `1` and use `ltAllScalar`.
        }
    }

    function full(uint256 n, uint256 m, N32x32 s) internal pure returns (M32x32 C) {
        C = mallocM32x32(n, m); // Allocate memory for matrix.

        fill_(C, s); // Fill matrix with `s`.
    }

    function fill_(M32x32 A, N32x32 s) internal pure {
        unchecked {
            (uint256 n, uint256 m, uint256 ptr) = header(A);
    ...
    /// @dev TODO optimize
    function neg(M32x32 A) internal pure returns (M32x32 C) {
        C = mulScalar(A, N32x32Lib.NEG_ONE);
    }

    function abs(M32x32 A) internal pure returns (M32x32 C) {
        unchecked {
            (uint256 n, uint256 m, uint256 ptrA) = header(A);
    ...
    function positive(M32x32 A) internal pure returns (M32x32 C) {
        unchecked {
            (uint256 n, uint256 m, uint256 ptrA) = header(A);
            ...
            while (ptrA != endA) {
                assembly {
                    let aX4 := mload(ptrA) // Load 4 values from `A` at `ptrA`.

                    // Zero out all negative numbers.
                    let cX4 := and(aX4, not(mul(shr(63, and(aX4, INT64_SIGN_X4)), UINT64_MAX)))
            ...
            if (rest != 0) {
                assembly {
                    ...
                    let mask := not(shr(mul(rest, 8), not(0)))
                    // Load 4 values from `A` at `ptrA`.
                    let aX4 := and(mload(ptrA), mask) // Load 4 values from `A` at `ptrA`.
                    // Zero out all negative numbers.
                    let cX4 := and(aX4, not(mul(shr(63, and(aX4, INT64_SIGN_X4)), UINT64_MAX)))
    ...
    function isNegative(M32x32 A) internal pure returns (M32x32 C) {
        unchecked {
            (uint256 n, uint256 m, uint256 ptrA) = header(A);
            ...
            while (ptrA != endA) {
                assembly {
                    let aX4 := mload(ptrA) // Load 4 values from `A` at `ptrA`.
                    let cX4 := shr(63, and(aX4, INT64_SIGN_X4)) // Mask sign and shift right.
    ...
    function isPositive(M32x32 A) internal pure returns (M32x32 C) {
        unchecked {
            (uint256 n, uint256 m, uint256 ptrA) = header(A);
            ...
            while (ptrA != endA) {
                assembly {
                    // Load 4 values from `A` at `ptrA`.
                    let aX4 := mload(ptrA)
                    // Mask sign and shift right.
                    let cX4 := shr(31, and(xor(aX4, INT64_SIGN_X4), INT64_SIGN_X4))
    ...
    function T(M32x32 A) internal pure returns (M32x32 C) {
        C = transpose(A);
    }

    function transposeNaive(M32x32 A) internal pure returns (M32x32 C) {
        unchecked {
    ...
    function transpose(M32x32 A) internal pure returns (M32x32 C) {
        unchecked {
    ...
    function map(M32x32 A, function (uint256) returns (uint256) fn) internal returns (M32x32 C) {
        unchecked {
            (uint256 n, uint256 m, uint256 ptrA) = header(A);
    ...
    function copy(M32x32 A) internal view returns (M32x32 C) {
        unchecked {
            (uint256 n, uint256 m, uint256 ptrA) = header(A);
    ...
    // function fromEncodedPacked_(bytes memory dataBytes) internal pure returns (M32x32 C) {
    //     C = M32x32Lib.fromEncodedPacked_(dataBytes, 1, dataBytes.length / 8);
    // }

    // function fromEncoded(bytes memory dataBytes, uint256 n, uint256 m) internal pure returns (M32x32 C) {
    //     unchecked {
    ...
    function bytes_(M32x32 A) internal pure returns (bytes memory dataBytes) {
        uint256 ptr = ref(A);
    ...
    function fromM32x32Bytes_(bytes memory dataBytes) internal pure returns (M32x32 C) {
        unchecked {
    ...
    function fromUintArray(uint256[][] memory data) internal pure returns (M32x32 C) {
        unchecked {
            ...
            if (carry > uint256(INT32_MAX)) revert N32x32Lib.Overflow();
        }
    }

    function fromUintEncoded(bytes memory dataBytes) internal pure returns (M32x32 C) {
        unchecked {
    ...
    function fromUintPtrTo_(uint256 ptr, M32x32 C) internal pure {
        unchecked {
    ...
    function fromIntEncoded(bytes memory dataBytes) internal pure returns (M32x32 C) {
        unchecked {
            uint256 m = dataBytes.length / 32;
    ...
    function fromIntPtrTo_(uint256 ptr, M32x32 C) internal pure {
        unchecked {
            (uint256 n, uint256 m, uint256 ptrC) = header(C);
            // Loop over all `n * m` elements of 32 bytes.
            uint256 size = n * m * 32;
            ...
            if (carry > UINT32_MAX) revert N32x32Lib.Overflow();
        }
    }

    function fromIntArray(int256[][] memory data) internal pure returns (M32x32 C) {
        unchecked {
            uint256 n = data.length;
    ...
    function fromUintArray(uint8[3][3] memory data) internal pure returns (M32x32 C) {
    ...
    function fromUintArray(uint8[4][2] memory data) internal pure returns (M32x32 C) {
        uint256 ptr;
    ...
    function fromUintArray(uint8[4][4] memory data) internal pure returns (M32x32 C) {
        uint256 ptr;
    ...
    function fromUintArray(uint256[2][2] memory data) internal pure returns (M32x32 C) {
        uint256 ptr;
    ...
    function fromUintArray(uint256[3][3] memory data) internal pure returns (M32x32 C) {
        uint256 ptr;
    ...
    function fromUintPtr(uint256 ptr, uint256 n, uint256 m) internal pure returns (M32x32 C) {
        C = mallocM32x32(n, m);

        fromUintPtrTo_(ptr, C);
    }

    function fromUint256PtrToUnsafe_(uint256 ptr, M32x32 C) internal pure {
        unchecked {
            (uint256 n, uint256 m, uint256 ptrC) = header(C);
            // Loop over all `n * m` elements of 32 bytes.
            uint256 size = n * m * 32;
    ...
    function packWordUnsafe(N32x32 a, N32x32 b, N32x32 c, N32x32 d) internal pure returns (uint256 word) {
    ...
    function packWordUnsafe(uint256 a, uint256 b, uint256 c, uint256 d) internal pure returns (uint256 word) {
